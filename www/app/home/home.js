var app = angular.module('app.home', [])
  .config(function($stateProvider) {
    ...
    // not sure why this route should exist?
    // $stateProvider.state('about', {
    //     url: '/about',
    //     templateUrl: 'login.html',
    // });
  })
  .controller('map-controller', function($scope, $window, $state, authFactory, mapFactory) {
    ...
    var map = L.map('map', {
      ...
      minZoom: 12
    }).setView([37.789, -122.414], 14);

    L.tileLayer('http://api.tiles.mapbox.com/v3/imtiazmajeed.j17fdf0d/{z}/{x}/{y}.png', {
      maxZoom: 18
    }).addTo(map);
    ...
    $scope.addHere = function(distance) {
      if ($scope.tag !== '') {
        ...
        L.circleMarker(here, {
          color: '#DB5A55',
          opacity: 0
        }).setRadius(0).bindLabel(($scope.tag), {
          noHide: true
        }).addTo(map);
      ...
      if ($scope.tag !== '') {
        ...
        request.onload = function() {
          ...
          for (var tag in tags) {
            ...
            //insert circle
            new L.circle(latlng, 40, {
              color: '#DB5A55',
              weight: 2,
              opacity: .8
            }).addTo(map);
            //insert circle marker so we can always show label
            var marker = L.circleMarker([latlng[0], markerlng], {
              color: '#DB5A55',
              opacity: 0
            }).setRadius(0).bindLabel(unescape(tags[tag].tag), {
              noHide: true
            }).addTo(map);
          }
        };
    ...
    $scope.saveTags = function() {
      ...
      tags.googleId = authFactory.user.id;
        ...
        if (request.status >= 200 && request.status < 400) {
          ...
          for (var coord in allCoords) {
            ...
            var marker = L.circleMarker([latlng[0], markerlng], {
              color: '#DB5A55',
              opacity: 0
            }).setRadius(0).bindLabel(unescape(allCoords[coord].tag), {
              noHide: true
            }).addTo(map);
          }
        }
    ...
    $scope.personalSwitch = function() {
      ...
      request.send(JSON.stringify(authFactory.user.id));
    };
